...
//结算银行卡照片
updeta_img("#clipArea3",428,321,"#file_new3","#view3","#clipBtn3",$('.cover-wrap3'));
//店铺头像
updeta_img("#clipArea4",428,321,"#file_new4","#view4","#clipBtn4",$('.cover-wrap4'));
//经营场所照片

//收银台招牌照片
updeta_img("#clipArea5",428,321,"#file_new5","#view5","#clipBtn5",$('.cover-wrap5'));
...
function updeta_img(clipArea,num_1,num_2,file, view, ok, class_name){
    ...
    new bjj.PhotoClip(clipArea, {
        ...
        clipFinish: function(dataURL) {
            ...
            if(clipArea == "#clipArea") {//营业执照
                $.ajax({
                    url: baseURL + "operation/tvyicoojinjian/uploadImageBase64",
                    data: {
                ...
                $.ajax({
                 ...
                 $.ajax({
                     ...
                     type: "post",
                     success: function(r) {
                          vm.tVyicooJinjian.shopPic = r.path;
                         $(view).css('background', 'url(' + r.imageNginxPath + vm.tVyicooJinjian.shopPic + ')');
                     }
               });
            }else if(clipArea == "#clipArea5") {//收银台招牌照
                 $.ajax({
                     url: baseURL + "operation/tvyicoojinjian/uploadImageBase64",
                     data: {
                         "imageBase64": dataURL.split(',')[1]
                     ...
                     type: "post",
                     success: function(r) {
                          vm.tVyicooJinjian.extraPic2 = r.path;
                         $(view).css('background', 'url(' + r.imageNginxPath + vm.tVyicooJinjian.extraPic2 + ')');
                     }
               });
            }
        }
