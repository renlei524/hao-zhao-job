$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [
			...
			{ label: '商户名称', name: 'creatorName', index: 'creator_name', width: 80 },
			...
			{ label: '状态', name: 'status', width: 80, formatter: function(value, options, row){
                    return value === 1 ?
                        '<span class="label label-success">发放中</span>' :
...
var vm = new Vue({
	...
	methods: {
		...
		add: function(){
            ...
            $('#view3').css('background', '').css("background", "url(/statics/img/default.png)").css('background-size','100% 100%');
		...
		del: function (event) {
			...
			for(var index in ids) {
                if($("#" + ids[index]).find("td").eq(4).attr("title") != "平台") {
                    alert("只能删除平台优惠券！");
                    return;
                }
            }
		...
		stop: function (event) {
            ...
            for(var index in ids) {
                if($("#" + ids[index]).find("td").eq(4).attr("title") != "平台") {
                    alert("只能禁用平台优惠券！");
                    return;
                } else if ($("#" + ids[index]).find("td").eq(13).attr("title") != "发放中") {
                    alert("只能禁用发放中优惠券！");
                    return;
                }
            }
            confirm('确定要禁用选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "operation/twbcard/stop",
                    contentType: "application/json",
                    data: JSON.stringify(ids),
                    success: function(r){
                        if(r.code == 0){
