$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			// { label: '提现类型', name: 'enchashmentType', index: 'enchashment_type', width: 80 , formatter: function(item, index){
             //        if(item === 1){
             //            return '<span class="label label-primary">微信</span>';
             //        }
             //        if(item === 2){
             //            return '<span class="label label-success">银行卡</span>';
             //        }
             //        if(item === 3){
	...
	methods: {
		...
		reload: function (event) {
			...
			$("#jqGrid").jqGrid('setGridParam',{
                postData:{'merchantName':vm.q.objectName},
                page:1
            }).trigger("reloadGrid");
             $("#text1").attr("status", "Y");
